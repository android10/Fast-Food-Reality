...
//	private MixVector oMarker = new MixVector();
	...
	//android10 added
	public Marker(String title, String address, String phonenumber, double latitude, double longitude, double altitude, String URL) {
		super();
...
//				}
	...
	public void setDistance(float distanceMeters){
		...
		this.distance = format.format(Float.valueOf(distanceMeters / 1000f).doubleValue()).toString();
		
//		Float calcDistance = distanceMeters / 1000f;
//		this.distance = calcDistance.toString();
	}
...
//	}
	...
	public void draw(PaintScreen dw) {
		...
		//android10 modified (remove hard code)
		if (textBlock == null) {
			StringBuilder distanceKm = new StringBuilder();
			distanceKm.append("DIST. ");
			distanceKm.append(distance);
			distanceKm.append(" KM");
			distanceKm.append("\r\n");
			distanceKm.append(address);
			textBlock = new TextObj(distanceKm.toString(), Math.round(maxHeight / 3f) + 2,
					160, dw);			
		}
		...
		if (isVisible) {
			...
			dw.paintIconAR(cMarker.x, cMarker.y, maxHeight / 1.5f);
	...
	public boolean fClick(float x, float y, MixContext ctx, MixState state) {
		...
		if (isClickValid(x, y)) {
			evtHandled = state.handleEvent(ctx, title, address, phonenumber, URL);
		}
		return evtHandled;
	}
}
